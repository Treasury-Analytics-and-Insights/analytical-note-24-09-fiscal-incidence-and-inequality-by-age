...
figure_filename <- "../../figures/Figure08_NetFiscalImpact.png"

figures_data <- fread(figures_data_filename)

data_subset <-
  figures_data[(Statistic == "Average Net Fiscal Impact" &
                  `Sharing assumption` == "Intra-family mOECD (Indirect tax only)") |
               (Statistic %in% c("Average Income support", "Average Direct taxes",
                                 "Average Education spending", "Average Health spending") &
...
nfi_data_long[note == "Suppressed", amount := 0]
nfi_data_long[, note := NULL]

nfi_data_long[quantity %like% "^Average", quantity := gsub("Average", "Mean", quantity)]
nfi_data_long[quantity %like% "Net", quantity := gsub("Net\\s", "", quantity)]

nfi_data_long[, quantity := tolower(gsub("\\s", "_", quantity))]

nfi_data_long <- rbind(nfi_data_long[, .(quantity, age, amount)],
                       nfi_data_long[quantity == "mean_fiscal_impact",
                                     .(quantity = "mean_fiscal_impact_ase", age,
                                       amount = amount_ase)])

nfi_data_long[, age := factor(age, levels = unique(age))]

nfi_data_wide <- dcast(nfi_data_long, age ~ quantity)

nfi_plot_data <-
  melt(nfi_data_wide[, .(age,
...
setorder(nfi_plot_data, variable, age)

nfi_age_labels <- nfi_plot_data[, levels(age)]

p <-
  ggplot(nfi_plot_data, aes(x = as.integer(age), y = value, fill = variable)) +
  ...
  geom_errorbar(aes(ymin = fiscal_impact - fiscal_impact_ase,
                ...
                width = 0.2) +
  scale_x_continuous(breaks = 1:length(nfi_age_labels), labels = nfi_age_labels) +
