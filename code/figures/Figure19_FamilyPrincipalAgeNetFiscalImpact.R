...
figure_filename <- "../../figures/Figure19_FamilyPrincipalAgeNetFiscalImpact.png"

figures_data <- fread(figures_data_filename)

data_subset <-
  figures_data[Statistic %in%
                 c("Average Net Fiscal Impact", "Average Income support", "Average Direct taxes",
                   "Average Indirect taxes", "Average In-kind spending") &
...
famnfi_data_long[quantity %like% "^Average",
                 quantity := gsub("Average", "Mean", quantity)]
famnfi_data_long[quantity %like% "Net",
                 quantity := gsub("Net\\s", "", quantity)]

famnfi_data_long[, quantity := tolower(gsub("(\\s|-)", "_", quantity))]

famnfi_data_long <- rbind(famnfi_data_long[, .(quantity, age, amount)],
                          famnfi_data_long[quantity == "mean_fiscal_impact",
                                           .(quantity = "mean_fiscal_impact_ase",
                                             age,
                                             amount = amount_ase)])

famnfi_data_long[, age := factor(age, levels = unique(age))]

famnfi_data_wide <- dcast(famnfi_data_long, age ~ quantity)

famnfi_plot_data <-
  melt(famnfi_data_wide[, .(age,
...
famnfi_plot_data[variable != "income_support",
                 `:=`(fiscal_impact = NA_real_, fiscal_impact_ase = NA_real_)]

famnfi_plot_data[, variable := factor(variable,
                                      levels = c("inkind_benefits", "income_support",
                                                 "indirect_tax", "direct_tax"))]

setorder(famnfi_plot_data, variable, age)

famnfi_age_labels <- famnfi_plot_data[, levels(age)]

p <-
  ggplot(famnfi_plot_data, aes(x = as.integer(age), y = value, fill = variable)) +
  ...
  geom_errorbar(aes(ymin = fiscal_impact - fiscal_impact_ase,
                ...
                width = 0.2) +
  scale_x_continuous(breaks = 1:length(famnfi_age_labels), labels = famnfi_age_labels) +
  scale_y_continuous(limits = c(-3.6e4, 6e4), breaks = seq(-3e4, 6e4, 3e4),
                     labels = scales::dollar_format()) +
  labs(x = "Age group (family principal earner)",
       y = "Average benefit or tax per family ($2019)") +
