...
## effects for individuals under the assumption of intra-family mOECD sharing.
...
fis_imp <- totals_and_means_with_ase(fis_age, "P_Fiscal_Impact_FIS", "fis_imp")
fimp_shr <- totals_and_means_with_ase(fis_age, "P_Fiscal_Impact_FIS_shared", "fimp_shr")
...
intrafam_moecd_redist_effects <- re_md[re_df, on = .(age_bin5, N)]


# Round
intrafam_moecd_fiscal_cpts_rounded <-
  round_totals_and_means(intrafam_moecd_fiscal_cpts,
                         dont_suppress = c("total_fis_imp", "mean_fis_imp",
                                           "total_fimp_shr", "mean_fimp_shr"))
intrafam_moecd_ginis_rounded <- round_ineq_stats(intrafam_moecd_ginis)
intrafam_moecd_redist_effects_rounded <- round_ineq_stats(intrafam_moecd_redist_effects)


# Output
fwrite(intrafam_moecd_fiscal_cpts,
       file.path(OUTPUT_DIR, "intrafam_moecd_fiscal_cpts.csv"))
fwrite(intrafam_moecd_fiscal_cpts_rounded,
       file.path(OUTPUT_DIR, "intrafam_moecd_fiscal_cpts_rounded.csv"))

fwrite(intrafam_moecd_ginis,
       file.path(OUTPUT_DIR, "intrafam_moecd_ginis.csv"))
fwrite(intrafam_moecd_ginis_rounded,
       file.path(OUTPUT_DIR, "intrafam_moecd_ginis_rounded.csv"))

fwrite(intrafam_moecd_redist_effects,
       file.path(OUTPUT_DIR, "intrafam_moecd_redist_effects.csv"))
fwrite(intrafam_moecd_redist_effects_rounded,
       file.path(OUTPUT_DIR, "intrafam_moecd_redist_effects_rounded.csv"))
